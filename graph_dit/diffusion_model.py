...
from models.enhanced_transformer import EnhancedDenoiser
...
from constraints.chemical_constraints import (
    ChemicalConstraintValidator,
    ConstraintGuidedLoss,
    AdaptiveConstraintScheduler,
    ConstraintGuidedSampler
)
...
class Graph_DiT(pl.LightningModule):
    def __init__(self, cfg, dataset_infos, train_metrics, sampling_metrics, visualization_tools):
        ...
        # Choose enhanced or original model based on config
        use_enhanced = getattr(cfg.model, 'use_enhanced', True)

        if use_enhanced:
            self.model = EnhancedDenoiser(
                ...
                use_edge_aware_attention=getattr(cfg.model, 'use_edge_aware_attention', True),
                use_cross_attention=getattr(cfg.model, 'use_cross_attention', True),
                use_adaptive_weighting=getattr(cfg.model, 'use_adaptive_weighting', True),
            )
        else:
        ...
        # Initialize fast sampling and constraint components
        use_fast_sampling = getattr(cfg.model, 'use_fast_sampling', True)
        use_constraints = getattr(cfg.model, 'use_constraints', True)

        if use_fast_sampling:
            self.fast_sampler = DDIMSampler(
                model=self,
                noise_schedule=self.noise_schedule,
                transition_model=self.transition_model,
                fast_steps=getattr(cfg.model, 'fast_steps', 50),
                eta=getattr(cfg.model, 'ddim_eta', 0.0)
            )

        if use_constraints:
            self.constraint_validator = ChemicalConstraintValidator(device=self.device)
            self.constraint_loss = ConstraintGuidedLoss(
                validator=self.constraint_validator,
                constraint_weights={
                    'valence': getattr(cfg.model, 'valence_weight', 10.0),
                    'connectivity': getattr(cfg.model, 'connectivity_weight', 5.0),
                    'symmetry': getattr(cfg.model, 'symmetry_weight', 1.0),
                }
            )
            self.constraint_scheduler = AdaptiveConstraintScheduler(
                initial_strength=getattr(cfg.model, 'constraint_initial', 0.1),
                final_strength=getattr(cfg.model, 'constraint_final', 1.0),
                warmup_steps=getattr(cfg.model, 'constraint_warmup', 1000)
            )

            if use_fast_sampling:
                self.constraint_guided_sampler = ConstraintGuidedSampler(
                    base_sampler=self.fast_sampler,
                    constraint_validator=self.constraint_validator,
                    guidance_strength=getattr(cfg.model, 'constraint_guidance', 1.0)
                )
    ...
    def sample_fast(self, batch_size: int, y: torch.Tensor, node_mask: torch.Tensor,
                   use_constraints: bool = True, guidance_scale: float = None) -> list:
        """
        Fast sampling using DDIM with optional constraint guidance

        Args:
            batch_size: Number of molecules to sample
            y: Conditional information
            node_mask: Valid node mask
            use_constraints: Whether to use constraint guidance
            guidance_scale: Classifier-free guidance scale

        Returns:
            List of sampled molecules
        """
        if hasattr(self, 'constraint_guided_sampler') and use_constraints:
            # Use constraint-guided fast sampling
            molecules = self.constraint_guided_sampler.sample_with_constraints(
                batch_size=batch_size,
                y=y,
                node_mask=node_mask,
                guidance_scale=guidance_scale or self.guide_scale,
                device=self.device
            )
        elif hasattr(self, 'fast_sampler'):
            # Use regular fast sampling
            molecules = self.fast_sampler.sample_fast(
                batch_size=batch_size,
                y=y,
                node_mask=node_mask,
                guidance_scale=guidance_scale or self.guide_scale,
    ...
    def compute_constraint_loss(self, pred_X: torch.Tensor, pred_E: torch.Tensor,
        ...
        if hasattr(self, 'constraint_loss'):
            return self.constraint_loss.compute_constraint_loss(pred_X, pred_E, node_mask)
        else:
            return torch.zeros(pred_X.shape[0], device=pred_X.device)

    def get_constraint_strength(self, step: int) -> float:
        """Get current constraint strength"""
        if hasattr(self, 'constraint_scheduler'):
            return self.constraint_scheduler.get_constraint_strength(step)
        else:
            return 0.0
